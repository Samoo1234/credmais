...
export default function SolarEnergySubscription() {
    ...
    const [data, setData] = useState<SolarEnergyData | null>(null);
    ...
    useEffect(() => {
        const fetchSettingsAndData = async () => {
            try {
                ...
                // Buscar dados da seção (via fetch para a API recém-criada, ou direto no supabase. Aqui vamos usar o supabase client diretamente já que temos acesso)
                const { data: sectionData, error } = await supabase.from('solar_energy_section').select('*').single();
    ...
    // Dados de fallback caso ainda não tenham sido carregados ou haja erro
    const displayData = data || {
        badge_text: 'Sustentabilidade',
        title: 'Assinatura de Energia Solar',
        description: 'Siga estes três passos simples para começar a economizar na sua conta de luz utilizando energia limpa.',
        steps: [
            {
                number: '1',
        ...
        <section id="energia-solar" className="py-20 lg:py-28 bg-gray-50 w-full">
            <div style={{ ...containerStyle, paddingTop: '3rem' }}>
                <div className="text-center max-w-3xl mx-auto mb-16" style={{ marginTop: '3rem' }}>
                    <span style={{ display: 'inline-block', padding: '0.5rem 1.5rem', background: 'linear-gradient(to right, #FC4C00, #FF7033)', color: 'white', fontSize: '0.875rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderRadius: '9999px', marginBottom: '1rem' }}>
                        {displayData.badge_text}
                    </span>
                    <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-[#29577E] mb-6">
                        {displayData.title}
                    </h2>
